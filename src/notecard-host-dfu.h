...
struct DFUConfig {
    ...
    /**
     * @brief How frequently to send requests to the Notecard. This aligns with the request frequency present in note-c.
     */
    constexpr static std::chrono::milliseconds DEFAULT_REQUEST_INTERVAL = 250ms;

    /**
     * @brief How often to check whether a DFU update is available.
     */
    constexpr static std::chrono::milliseconds DEFAULT_CHECK_DFU_AVAILABLE_INTERVAL = 10min;

    /**
     * @brief How long to wait for the Notecard to enter DFU mode.
     */
    constexpr static std::chrono::milliseconds DEFAULT_WAIT_FOR_DFU_TIMEOUT = 2min;
    ...
    */
    ...
    std::chrono::milliseconds waitForDFUTimeout = DEFAULT_WAIT_FOR_DFU_TIMEOUT;
...
/**
 * @brief "Feeds" the DFU task, which breaks up DFU operations into small pieces so that it is non-blocking as a whole. Typically, each
